...
Parquet import with `importFromParquetFile()` is only available in Node.js and requires Exasol 2026.1 or later. The driver streams the original local Parquet file; it does not parse, convert, or infer its schema.
