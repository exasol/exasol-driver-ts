...
The `ci-build.yml` workflow runs quality gates for pull requests and pushes to `main`. It runs the full test suite and SonarCloud scan for the latest Exasol release, and runs integration tests for the supported 2025.1 and 8 LTS releases. The workflow passes the selected Docker image through `EXASOL_DOCKER_VERSION`, while local integration test runs default to the latest release. The `release.yml` workflow runs the default Node.js and Exasol verification gates sequentially before packaging and publishing.
