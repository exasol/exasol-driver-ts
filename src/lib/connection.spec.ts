...
describe('connection', () => {
  ...
  describe('sendCommandWithNoResult()', () => {
    it('sends a command without awaiting a response', async () => {
  ...
  describe('sendCommand()', () => {
    it('rejects when the socket is closing', async () => {
      const sendFunction = jest.fn();
      const mockSocket = {
        send: sendFunction,
        readyState: 2,
    ...
    it('sends a command and resolves its response', async () => {
      const sendFunction = jest.fn();
      const mockSocket = {
        send: sendFunction,
        readyState: 1,
    ...
    it('sends abortQuery immediately while a response-producing command is pending', async () => {
      ...
      expect(mockSocket.sentCommands).toEqual([{ command: 'execute', sqlText: 'select 1' }, { command: 'abortQuery' }]);
  ...
  describe('breakConnection()', () => {
    // [utest->dsn~runtime-inflight-websocket-failure~1]
    it('rejects active and queued commands when explicitly broken', async () => {
      const mockSocket = createManualMockSocket();
      const connection = new Connection(mockSocket, new Logger(), 'test');
      ...
      await expect(firstCommand).rejects.toThrow('connection reset');
      await expect(secondCommand).rejects.toThrow('connection reset');
      expect(connection.active).toBe(false);
      expect(connection.broken).toBe(true);
      expect(mockSocket.closed).toBe(true);
    });
